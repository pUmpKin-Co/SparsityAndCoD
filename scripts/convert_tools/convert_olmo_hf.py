...
from olmo.custom_hf_model import CustomOlmo2ForCausalLM
from olmo.custom_hf_model_config import CustomOlmo2Config
...
def write_model(model_path, input_base_path, tokenizer_path=None, safe_serialization=True, fix_eos_token_id=True, norm_after=False, layer_norm_scale=False, attention_center=False, center_method="attn", attention_layer_norm=False):
    ...
    for layer_i in range(n_layers):
        ...
        if norm_after:
            state_dict[f"model.layers.{layer_i}.post_attention_layernorm.weight"] = loaded[f"transformer.blocks.{layer_i}.attn_norm.weight"]
            state_dict[f"model.layers.{layer_i}.post_feedforward_layernorm.weight"] = loaded[f"transformer.blocks.{layer_i}.ff_norm.weight"]
        else:
            state_dict[f"model.layers.{layer_i}.pre_attention_layernorm.weight"] = loaded[f"transformer.blocks.{layer_i}.attn_norm.weight"]
            state_dict[f"model.layers.{layer_i}.pre_feedforward_layernorm.weight"] = loaded[f"transformer.blocks.{layer_i}.ff_norm.weight"]
        
        if attention_layer_norm:
            state_dict[f"model.layers.{layer_i}.self_attn.q_norm.weight"] = loaded[f"transformer.blocks.{layer_i}.q_norm.weight"]
            state_dict[f"model.layers.{layer_i}.self_attn.k_norm.weight"] = loaded[f"transformer.blocks.{layer_i}.k_norm.weight"]
    ...
    model = CustomOlmo2ForCausalLM.from_pretrained(tmp_model_path, dtype=torch.bfloat16)
...
def main():
    ...
    parser.add_argument("--norm_after", action="store_true", help="Whether to apply layer norm after the attention and feedforward layers.")
    parser.add_argument("--layer_norm_scale", action="store_true", help="Whether to scale the layer norm by the layer index.")
    parser.add_argument("--attention_center", action="store_true", help="Whether to center the attention output.")
    parser.add_argument("--center_method", choices=["attn", "value"], help="The method to use to center the attention output.", default="attn")
    parser.add_argument("--attention_layer_norm", action="store_true", help="Whether to apply layer norm to the attention output.")
