...
def should_checkpoint_block(
    ...
    if strategy is None:
        ...
        or (
            strategy == ActivationCheckpointingStrategy.one_in_three
            and block_idx % 3 == 0
        )
        or (
        ...
        or (
            strategy == ActivationCheckpointingStrategy.one_in_eight
            and block_idx % 8 == 0
        )
        or (
...
class LayerNormBase(nn.Module):
    ...
    def build(
        ...
        if config.layer_norm_type == LayerNormType.default:
            return LayerNorm(
                config, size=size, low_precision=False, **kwargs
            )
        elif config.layer_norm_type == LayerNormType.low_precision:
            return LayerNorm(
                config, size=size, low_precision=True, **kwargs
            )
    ...
    def _cast_if_autocast_enabled(
        ...
        if tensor.device.type == "cuda" and torch.is_autocast_enabled():
            ...
            tensor.device.type == "cpu" and torch.is_autocast_cpu_enabled()
        ):
            return tensor.to(
                dtype=(
                    dtype
...
class LayerNorm(LayerNormBase):
    ...
    def __init__(
        ...
        super().__init__(
            config, size=size, elementwise_affine=elementwise_affine
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if self.low_precision:
            ...
            with torch.autocast(
                enabled=False, device_type=module_device.type
            ):
                return F.layer_norm(
                    downcast_x,
    ...
    def get_rotary_embedding(
        ...
        with torch.autocast(device.type, enabled=False):
            ...
            inv_freq = 1.0 / (
                ...
                ** (
                    torch.arange(
                        0, dim, 2, device=device, dtype=torch.float
                    )
                    / dim
                )
            )
...
def alibi_attention_bias(
    ...
    # shape: (n_heads,)
    m = torch.arange(
        1, config.n_heads + 1, dtype=torch.float, device=device
    )
...
class OLMoBlock(nn.Module):
    ...
    def reset_parameters(self):
        ...
        if self.config.init_fn == InitFnType.normal:
            ...
            attn_out_std = ff_out_std = self.config.init_std / math.sqrt(
                2.0 * self.config.n_layers
            )
            cutoff_factor = self.config.init_cutoff_factor or 3.0

        if self.config.init_fn == InitFnType.kaiming_uniform:
            torch.nn.init.kaiming_uniform_(self.attn_out.weight)
            if self.attn_out.bias is not None:
                torch.nn.init.zeros_(self.attn_out.bias)
            torch.nn.init.kaiming_uniform_(self.ff_out.weight)
            if self.ff_out.bias is not None:
                torch.nn.init.zeros_(self.ff_out.bias)
        elif self.config.init_fn == InitFnType.kaiming_normal:
            torch.nn.init.kaiming_normal_(self.attn_out.weight)
            torch.nn.init.kaiming_normal_(self.ff_out.weight)
            if self.ff_out.bias is not None:
                torch.nn.init.zeros_(self.ff_out.bias)
            if self.attn_out.bias is not None:
                torch.nn.init.zeros_(self.attn_out.bias)
        elif self.config.init_fn == InitFnType.xavier_uniform:
            torch.nn.init.xavier_uniform_(self.attn_out.weight)
            torch.nn.init.xavier_uniform_(self.ff_out.weight)
            if self.ff_out.bias is not None:
                torch.nn.init.zeros_(self.ff_out.bias)
            if self.attn_out.bias is not None:
                torch.nn.init.zeros_(self.attn_out.bias)
        elif self.config.init_fn == InitFnType.xavier_normal:
            torch.nn.init.xavier_normal_(self.attn_out.weight)
            torch.nn.init.xavier_normal_(self.ff_out.weight)
            if self.ff_out.bias is not None:
                torch.nn.init.zeros_(self.ff_out.bias)
            if self.attn_out.bias is not None:
                torch.nn.init.zeros_(self.attn_out.bias)
    ...
    def set_activation_checkpointing(
        self,
        strategy: Optional[ActivationCheckpointingStrategy],
        checkpoint_func: Optional[Callable] = None,
    ):
        if strategy == ActivationCheckpointingStrategy.fine_grained:
            self._activation_checkpoint_fn = (
                checkpoint_func
                or activation_checkpoint_function(self.config)
            )
        else:
    ...
    def _scaled_dot_product_attention(
        ...
        if max_doc_len is not None and cu_doc_lens is not None:
            assert (
                self.flash_attn_varlen_func is not None
            ), "flash-attn is required for document masking"
            assert (
                attn_mask is None
            ), "attn-mask is currently not supported with document masking"
    ...
    def attention(
        ...
        if self.config.attn_minus:
            att = att - att.mean(dim=-2, keepdim=True)
        elif self.config.mindgap:
            att = att - v.mean(dim=-2, keepdim=True)
    ...
    def forward(
        ...
        max_doc_len: Optional[int] = None,
        cu_doc_lens: Optional[torch.Tensor] = None,
        output_sublayer_statis: Optional[bool] = False,
    ) -> Tuple[torch.Tensor, Optional[Tuple[torch.Tensor, torch.Tensor]]]:
        raise NotImplementedError
    ...
    def build(
        ...
        if config.block_type == BlockType.sequential:
            return OLMoSequentialBlock(
                ...
                config,
                cache,
                layer_norm_scaling=layer_norm_scaling,
            )
        elif config.block_type == BlockType.llama:
            return OLMoLlamaBlock(
                layer_id,
                config,
                cache,
                layer_norm_scaling=layer_norm_scaling,
            ...
            ), "moe_config is required for MoE block"
            return OLMoMoEBlock(
                layer_id,
                config,
                cache,
...
class OLMoMoEBlock(OLMoBlock):
    def __init__(
        ...
        super().__init__(
            layer_id, config, cache, layer_norm_scaling=layer_norm_scaling
        )
        ...
        # TODO: build MoE Block -> Reset Parameter -> FSDP Wrapper
        # TODO: What is DT tensor and DeviceMesh?
        # TODO: What is Load Balancing Loss and Router Z Loss?
        # TODO: Deep understand the ParallelMoE
        self.ff_out = None
        self.act = None
        if config.moe_config.moe_type == MoEType.dropless_moe:
            self.ff = DroplessMoE(
                model_config=config,
        ...
        if config.peri_norm:
            self.attn_norm_post = LayerNorm.build(
                config, size=config.d_model
            )
            self.ff_norm_post = LayerNorm.build(config, size=config.d_model)
        else:
            self.attn_norm_post = nn.Identity()
            self.ff_norm_post = nn.Identity()
    ...
    def reset_parameters(self):
        ...
        if self.config.init_fn == InitFnType.normal:
            attn_out_std = router_std = attn_proj_std = expert_in_std = (
                expert_out_std
            ...
            attn_proj_std = 1 / math.sqrt(self.config.d_model)

            router_std = 1 / math.sqrt(self.config.d_model)
            expert_in_std = 1 / math.sqrt(
                self.config.d_model * self.config.moe_config.num_experts
            )
            expert_out_std = 1 / math.sqrt(
                2
                * self.config.moe_config.hidden_size
            ...
            attn_out_std = expert_out_std = (
                self.config.init_std / math.sqrt(2.0 * self.config.n_layers)
            )
            attn_proj_std = router_std = expert_in_std = (
                self.config.init_std
            )
            cutoff_factor = self.config.init_cutoff_factor or 3.0
        ...
        if self.config.init_fn == InitFnType.kaiming_uniform:
            torch.nn.init.kaiming_uniform_(self.attn_out.weight)
            torch.nn.init.kaiming_uniform_(self.att_proj.weight)
            torch.nn.init.kaiming_uniform_(self.ff.router.weight)
            torch.nn.init.kaiming_uniform_(self.ff.experts.mlp.w1.weight)
            torch.nn.init.kaiming_uniform_(self.ff.experts.mlp.w3.weight)
            torch.nn.init.kaiming_uniform_(self.ff.experts.mlp.w2.weight)
            if self.attn_out.bias is not None:
                torch.nn.init.zeros_(self.attn_out.bias)
            if self.att_proj.bias is not None:
                torch.nn.init.zeros_(self.att_proj.bias)
            if self.ff.experts.mlp.w1.bias is not None:
                torch.nn.init.zeros_(self.ff.experts.mlp.w1.bias)
            if self.ff.experts.mlp.w3.bias is not None:
                torch.nn.init.zeros_(self.ff.experts.mlp.w3.bias)
            if self.ff.experts.mlp.w2.bias is not None:
                torch.nn.init.zeros_(self.ff.experts.mlp.w2.bias)
        elif self.config.init_fn == InitFnType.kaiming_normal:
            torch.nn.init.kaiming_normal_(self.attn_out.weight)
            torch.nn.init.kaiming_normal_(self.att_proj.weight)
            torch.nn.init.kaiming_normal_(self.ff.router.weight)
            torch.nn.init.kaiming_normal_(self.ff.experts.mlp.w1.weight)
            torch.nn.init.kaiming_normal_(self.ff.experts.mlp.w3.weight)
            torch.nn.init.kaiming_normal_(self.ff.experts.mlp.w2.weight)
            if self.attn_out.bias is not None:
                torch.nn.init.zeros_(self.attn_out.bias)
            if self.att_proj.bias is not None:
                torch.nn.init.zeros_(self.att_proj.bias)
            ...
            torch.nn.init.xavier_uniform_(self.attn_out.weight)
            torch.nn.init.xavier_uniform_(self.att_proj.weight)
            torch.nn.init.xavier_uniform_(self.ff.router.weight)
            torch.nn.init.xavier_uniform_(self.ff.experts.mlp.w1.weight)
            torch.nn.init.xavier_uniform_(self.ff.experts.mlp.w3.weight)
            torch.nn.init.xavier_uniform_(self.ff.experts.mlp.w2.weight)
            if self.attn_out.bias is not None:
                torch.nn.init.zeros_(self.attn_out.bias)
            if self.att_proj.bias is not None:
                torch.nn.init.zeros_(self.att_proj.bias)
            ...
            torch.nn.init.xavier_normal_(self.attn_out.weight)
            torch.nn.init.xavier_normal_(self.att_proj.weight)
            torch.nn.init.xavier_normal_(self.ff.router.weight)
            torch.nn.init.xavier_normal_(self.ff.experts.mlp.w1.weight)
            torch.nn.init.xavier_normal_(self.ff.experts.mlp.w3.weight)
            torch.nn.init.xavier_normal_(self.ff.experts.mlp.w2.weight)
            if self.attn_out.bias is not None:
                torch.nn.init.zeros_(self.attn_out.bias)
            if self.att_proj.bias is not None:
                torch.nn.init.zeros_(self.att_proj.bias)
            ...
            init_normal(
                self.ff.experts.mlp.w1,
                std=expert_in_std,
                init_cutoff_factor=cutoff_factor,
                is_weight=True,
            )
            init_normal(
                self.ff.experts.mlp.w3,
                std=expert_in_std,
                init_cutoff_factor=cutoff_factor,
                is_weight=True,
            )
            init_normal(
                self.ff.experts.mlp.w2,
                std=expert_out_std,
                init_cutoff_factor=cutoff_factor,
                is_weight=True,
            )
        ...
        if self.config.peri_norm:
            self.attn_norm_post.reset_parameters()
            self.ff_norm_post.reset_parameters()

        if self.ff.shared_mlp is not None:
            if self.config.init_fn == InitFnType.normal:
                in_std = out_std = self.config.init_std

            elif self.config.init_fn == InitFnType.mitchell:
                out_std = 1 / (
                    math.sqrt(
                        2
                        * self.config.moe_config.shared_mlp.hidden_size
                        * (self.layer_id + 1)
                    )
                )
                ...
                out_std = self.config.init_std / math.sqrt(
                    2.0 * self.config.n_layers
                )
        ...
        if self.config.moe_config.shared_mlp is not None:
            if self.config.init_fn == InitFnType.kaiming_uniform:
                torch.nn.init.kaiming_uniform_(self.ff.shared_mlp.w1.weight)
                torch.nn.init.kaiming_uniform_(self.ff.shared_mlp.w3.weight)
                torch.nn.init.kaiming_uniform_(self.ff.shared_mlp.w2.weight)
                if self.ff.shared_mlp.w1.bias is not None:
                    torch.nn.init.zeros_(self.ff.shared_mlp.w1.bias)
                if self.ff.shared_mlp.w3.bias is not None:
                    torch.nn.init.zeros_(self.ff.shared_mlp.w3.bias)
                if self.ff.shared_mlp.w2.bias is not None:
                    torch.nn.init.zeros_(self.ff.shared_mlp.w2.bias)
            elif self.config.init_fn == InitFnType.kaiming_normal:
                torch.nn.init.kaiming_normal_(self.ff.shared_mlp.w1.weight)
                torch.nn.init.kaiming_normal_(self.ff.shared_mlp.w3.weight)
                torch.nn.init.kaiming_normal_(self.ff.shared_mlp.w2.weight)
                if self.ff.shared_mlp.w1.bias is not None:
                    torch.nn.init.zeros_(self.ff.shared_mlp.w1.bias)
                if self.ff.shared_mlp.w3.bias is not None:
                    torch.nn.init.zeros_(self.ff.shared_mlp.w3.bias)
                ...
                torch.nn.init.xavier_uniform_(self.ff.shared_mlp.w1.weight)
                torch.nn.init.xavier_uniform_(self.ff.shared_mlp.w3.weight)
                torch.nn.init.xavier_uniform_(self.ff.shared_mlp.w2.weight)
                if self.ff.shared_mlp.w1.bias is not None:
                    torch.nn.init.zeros_(self.ff.shared_mlp.w1.bias)
                if self.ff.shared_mlp.w3.bias is not None:
                    torch.nn.init.zeros_(self.ff.shared_mlp.w3.bias)
                ...
                torch.nn.init.xavier_normal_(self.ff.shared_mlp.w1.weight)
                torch.nn.init.xavier_normal_(self.ff.shared_mlp.w3.weight)
                torch.nn.init.xavier_normal_(self.ff.shared_mlp.w2.weight)
                if self.ff.shared_mlp.w1.bias is not None:
                    torch.nn.init.zeros_(self.ff.shared_mlp.w1.bias)
                if self.ff.shared_mlp.w3.bias is not None:
                    torch.nn.init.zeros_(self.ff.shared_mlp.w3.bias)
                ...
                init_normal(
                    self.ff.shared_mlp.w1,
                    std=in_std,
                    init_cutoff_factor=cutoff_factor,
                )
                init_normal(
                    self.ff.shared_mlp.w3,
                    std=in_std,
                    init_cutoff_factor=cutoff_factor,
                )
                init_normal(
                    self.ff.shared_mlp.w2,
                    std=out_std,
                    init_cutoff_factor=cutoff_factor,
                )
    ...
    @property
    def router(self):
        return self.ff.router

    @property
    def shared_mlp(self):
        return self.ff.shared_mlp

    @property
    def experts(self):
        return self.ff.experts

    @property
    def top_k(self):
        return self.ff.top_k

    def compute_metrics(
        self, reset: bool = True
    ) -> Dict[str, Tuple[torch.Tensor, Optional["ReduceType"]]]:
        return self.ff.compute_metrics(reset=reset)

    def reset_metrics(self):
        self.ff.reset_metrics()
    ...
    def forward(
        ...
        # apply norm before
        if output_sublayer_statis:
            layer_output = []

        if (
            not self.config.norm_after
            and self.config.mix_norm_factor is None
        ) or (
            self.config.mix_norm_factor is not None
            and self.layer_id
            >= np.floor(self.config.n_layers * self.config.mix_norm_factor)
        ):
        ...
        if (
            self.config.norm_after and self.config.mix_norm_factor is None
        ) or (
            self.config.mix_norm_factor is not None
            and self.layer_id
            < np.floor(self.config.n_layers * self.config.mix_norm_factor)
        ...
        if self.config.peri_norm and self.layer_norm_scaling:
            attn = self.attn_norm_post(att) * (
                self.config.moe_config.top_k / math.sqrt(self.layer_id + 1)
            )
            if output_sublayer_statis:
                layer_output.append(attn)
            x = x + self.dropout(attn)
        elif self.config.peri_norm:
            attn = self.attn_norm_post(att)
            if output_sublayer_statis:
                layer_output.append(attn)
            x = x + self.dropout(attn)
        ...
        x = self.ff(x, loss_div_factor=loss_div_factor)
        ...
        if self.config.peri_norm and self.layer_norm_scaling:
            x = self.dropout(
                self.ff_norm_post(x) * (self.config.moe_config.top_k / math.sqrt(self.layer_id + 1))
            )
        elif self.config.peri_norm:
            x = self.dropout(self.ff_norm_post(x))
        else:
            x = self.dropout(x)
...
class OLMoSequentialBlock(OLMoBlock):
    ...
    def reset_parameters(self):
        ...
        if self.config.init_fn == InitFnType.kaiming_uniform:
            torch.nn.init.kaiming_uniform_(self.att_proj.weight)
            torch.nn.init.kaiming_uniform_(self.ff_proj.weight)
            if self.ff_proj.bias is not None:
                torch.nn.init.zeros_(self.ff_proj.bias)
            if self.att_proj.bias is not None:
                torch.nn.init.zeros_(self.att_proj.bias)
        elif self.config.init_fn == InitFnType.kaiming_normal:
            torch.nn.init.kaiming_normal_(self.att_proj.weight)
            torch.nn.init.kaiming_normal_(self.ff_proj.weight)
            if self.ff_proj.bias is not None:
                torch.nn.init.zeros_(self.ff_proj.bias)
            if self.att_proj.bias is not None:
                torch.nn.init.zeros_(self.att_proj.bias)
            ...
            torch.nn.init.xavier_uniform_(self.att_proj.weight)
            torch.nn.init.xavier_uniform_(self.ff_proj.weight)
            if self.ff_proj.bias is not None:
                torch.nn.init.zeros_(self.ff_proj.bias)
            if self.att_proj.bias is not None:
                torch.nn.init.zeros_(self.att_proj.bias)
            ...
            torch.nn.init.xavier_normal_(self.att_proj.weight)
            torch.nn.init.xavier_normal_(self.ff_proj.weight)
            if self.ff_proj.bias is not None:
                torch.nn.init.zeros_(self.ff_proj.bias)
            if self.att_proj.bias is not None:
                torch.nn.init.zeros_(self.att_proj.bias)
    ...
    def forward(
        ...
        if (
            ...
            if self.layer_norm_scaling:
                h = h * (1 / math.sqrt(self.layer_id + 1))
        else:
            h = x
        ...
        if (
            ...
            if self.layer_norm_scaling:
                att = att * (1 / math.sqrt(self.layer_id + 1))

        # Add attention scores.
        # shape: (B, T, C)
        if self.config.peri_norm and self.layer_norm_scaling:
            attn = self.attn_norm_post(att) * (
                1 / math.sqrt(self.layer_id + 1)
            )
            if output_sublayer_statis:
                layer_output.append(attn)
            x = x + self.dropout(attn)
        ...
        if (
            ...
            if self._activation_checkpoint_fn is not None:
                ...
                x = self.ff_norm(x)
            if self.layer_norm_scaling:
                x = x * (1 / math.sqrt(self.layer_id + 1))
        ...
        if self.config.peri_norm and self.layer_norm_scaling:
            x = self.dropout(
                self.ff_norm_post(x) * (1 / math.sqrt(self.layer_id + 1))
            )
        elif self.config.peri_norm:
            x = self.dropout(self.ff_norm_post(x))
        else:
...
class OLMoLlamaBlock(OLMoBlock):
    ...
    def _scaled_dot_product_attention(
        ...
        if is_causal:
            ...
            attn_bias = get_causal_attention_bias(
    ...
    def forward(
        ...
        if self.layer_norm_scaling:
            x_normed = x_normed * (1 / math.sqrt(self.layer_id + 1))
...
class OLMoOutput(NamedTuple):
    ...
    layers_output: Optional[List[torch.Tensor]]
    """
...
class OLMoBlockGroup(nn.ModuleList):
    ...
    def forward(
        ...
        for block_idx, block in enumerate(self):
            layer_past = (
                None if layers_past is None else layers_past[block_idx]
            )
            block_idx += self.layer_offset
            if should_checkpoint_block(
                self.activation_checkpointing_strategy, block_idx
            ):
                # shape: (batch_size, seq_len, d_model)
                x, cache, layer_output = self._activation_checkpoint_fn(  # type: ignore
                    block,
                    x,
        ...
        if output_sublayer_statis:
            return x, attn_key_values, layers_output
        else:
            return x, attn_key_values, None
    ...
    def set_activation_checkpointing(
        ...
        for block in self:
            block.set_activation_checkpointing(
                strategy, checkpoint_func=checkpoint_func
            )
...
class OLMo(nn.Module):
    def __init__(self, config: ModelConfig, init_params: bool = True):
        ...
        blocks = [
            OLMoBlock.build(
                ...
                config,
                self.__cache,
                layer_norm_scaling=config.layer_norm_scaling,
            )
    ...
    def set_activation_checkpointing(
        ...
        if self.config.block_group_size != 1:
            for block_group in self.transformer.block_groups:
                block_group.set_activation_checkpointing(
                    strategy, checkpoint_func=checkpoint_func
                )
        else:
    ...
    def reset_parameters(self):
        ...
        if self.config.init_fn == InitFnType.normal:
            ...
            wte_std = self.config.emb_init_std or 1.0 / math.sqrt(
                self.config.d_model
            )
        ...
        if self.config.init_fn == InitFnType.kaiming_uniform:
            torch.nn.init.kaiming_uniform_(self.transformer.wte.weight)
        elif self.config.init_fn == InitFnType.kaiming_normal:
            torch.nn.init.kaiming_normal_(self.transformer.wte.weight)
        elif self.config.init_fn == InitFnType.xavier_uniform:
            torch.nn.init.xavier_uniform_(self.transformer.wte.weight)
        elif self.config.init_fn == InitFnType.xavier_normal:
            torch.nn.init.xavier_normal_(self.transformer.wte.weight)
        else:
            init_normal(
        ...
        if hasattr(self.transformer, "wpe"):
            ...
            if self.config.init_fn == InitFnType.kaiming_uniform:
                torch.nn.init.kaiming_uniform_(self.transformer.wpe.weight)
            elif self.config.init_fn == InitFnType.kaiming_normal:
                torch.nn.init.kaiming_normal_(self.transformer.wpe.weight)
            elif self.config.init_fn == InitFnType.xavier_uniform:
                torch.nn.init.xavier_uniform_(self.transformer.wpe.weight)
            elif self.config.init_fn == InitFnType.xavier_normal:
                torch.nn.init.xavier_normal_(self.transformer.wpe.weight)
            else:
                init_normal(
        ...
        if hasattr(self.transformer, "ff_out"):
            ...
            if self.config.init_fn == InitFnType.kaiming_uniform:
                torch.nn.init.kaiming_uniform_(
                    self.transformer.ff_out.weight
                )
                if self.transformer.ff_out.bias is not None:
                    torch.nn.init.zeros_(self.transformer.ff_out.bias)
            elif self.config.init_fn == InitFnType.kaiming_normal:
                ...
                torch.nn.init.xavier_normal_(self.transformer.ff_out.weight)
                if self.transformer.ff_out.bias is not None:
                    torch.nn.init.zeros_(self.transformer.ff_out.bias)
            else:
                init_normal(
    ...
    def forward(
        ...
        output_sublayer_statis: Optional[bool] = None,
        ...
        if (
            ...
            # NOTE: we need to initialize the attn bias in order for attn to work properly
        ...
        if self.config.block_group_size == 1:
            for block_idx, block in enumerate(self.transformer.blocks):
                ...
                if should_checkpoint_block(
                    ...
                    # shape: (batch_size, seq_len, d_model)
                    x, cache, layer_output = self._activation_checkpoint_fn(
                        block,
                        x,
                        attention_bias=attention_bias,
                        layer_past=layer_past,
                        ...
                        max_doc_len=max_doc_len,
                        cu_doc_lens=cu_doc_lens,
                        output_sublayer_statis=output_sublayer_statis,
                    )
                else:
                    # shape: (batch_size, seq_len, d_model)
                    x, cache, layer_output = block(
            ...
            for group_idx, block_group in enumerate(
                ...
                x, cache, layer_output = block_group(
                    x,
                    attention_bias=attention_bias,
                ...
                if output_sublayer_statis:
                    layers_output.extend(layer_output)
    ...
    def get_fsdp_wrap_policy(
        ...
        if wrap_strategy == FSDPWrapStrategy.by_block:
            ...
            def fsdp_wrap_fn(
                module, recurse: bool = True, nonwrapped_numel: int = 0
            ):
            ...
            def fsdp_wrap_fn(
                ...
                del nonwrapped_numel
                wrap = (
                    isinstance(module, (OLMoBlockGroup,))
                    or module in size_based_module_to_wrap
                )
                if recurse:
                    return True
    ...
    def num_activated_params(self, include_embedding: bool = True) -> int:
        """
        Get the number of activated parameters.
        For MoE models, this returns the number of parameters that are actually used
        during forward pass (non-MoE params + router params + shared MLP params + 
        top_k/num_experts fraction of expert params).
        For non-MoE models, this returns the same as num_params.
        """
        if not self.is_moe:
            return self.num_params(include_embedding=include_embedding)
        
        # For MoE models, calculate activated parameters
        total_params = 0
        expert_params = 0
        
        moe_config = self.config.moe_config
        assert moe_config is not None, "moe_config should not be None for MoE models"
        activation_ratio = moe_config.top_k / moe_config.num_experts
        
        for name, param in self.named_parameters():
            # Skip embeddings if requested
            if not include_embedding and (".wte." in name or ".wpe." in name):
                continue
            
            # Expert parameters: only count top_k/num_experts fraction
            if ".ff.experts." in name:
                expert_params += param.numel()
            else:
                # All other parameters (embeddings, attention, layer norms, router, shared_mlp) are always active
                total_params += param.numel()
        
        # Add the activated fraction of expert parameters
        total_params += int(expert_params * activation_ratio)
    ...
    def num_fwd_flops(self):
        ...
        attn_flops_per_token = (
            ...
            * 2
            * 2
            * (self.config.d_model * self.config.max_sequence_length)
        )
    ...
    def generate(
            ...
            if tokens_generated > 0:
                ...
                if attention_mask is not None:
                    ...
                    attention_mask = torch.cat(
                        (
                            attention_mask,
                            attention_mask.new_ones((group_size, 1)),
                        ),
                        dim=-1,
                    )
            else:
        ...
        initial_preds = input_ids.new_zeros(
            (batch_size,)
        ...
        if attention_bias is not None:
            state["attention_bias"] = attention_bias
        with torch.no_grad():
            token_ids, scores = beam_search.search(
                initial_preds, state, step
            )
    ...
    def from_checkpoint(
        ...
        if checkpoint_type == CheckpointType.unsharded:
            ...
            if (
                train_config.sharded_checkpointer
        ...
        if self.config.block_group_size != state_dict_block_group_size:
            ...
            if state_dict_block_group_size > 1:
                for key in list(state_dict.keys()):
                    if (
                        m := re.match(
                            r"transformer.block_groups\.(\d+)\.(\d+)\..*",
                            key,
                        )
                    ) is not None:
                        group_idx, group_block_idx = int(m.group(1)), int(
                            m.group(2)
                        )
                        block_idx = (
                            group_idx * state_dict_block_group_size
                        ) + group_block_idx
                        state_dict[
                            (
                                new_key := key.replace(
                        ...
                        new_keys_to_og_keys[new_key] = (
                            new_keys_to_og_keys.pop(key)
                        )
    ...
    def is_moe(self) -> bool:
        return self.config.block_type == BlockType.moe
    ...
    def compute_auxiliary_metrics(
        ...
        for block_idx, block in enumerate(self.transformer.blocks):
            block = cast(OLMoMoEBlock, block)
            block_metrics = block.compute_metrics(reset=reset)
            for metric_name, (
                metric_val,
                reduce_type,
            ) in block_metrics.items():
    ...
    def reset_auxiliary_metrics(self):
        for block in self.transformer.blocks.values():
            cast(OLMoMoEBlock, block).reset_metrics()
