...
class MemMapDataset(Dataset[Dict[str, Any]]):
    ...
    def _get_file_length(self, path, dtype=None) -> Tuple[PathOrStr, int]:
        ...
        chunk_bytes = item_size * self._chunk_size

        # Validate that file size is reasonable
        if total_file_size == 0:
            raise ValueError(f"Data file '{path}' is empty")

        if total_file_size < chunk_bytes:
            raise ValueError(
                f"Data file '{path}' is too small ({total_file_size} bytes) to contain even one chunk ({chunk_bytes} bytes)"
            )

        num_chunks = total_file_size // chunk_bytes
        if num_chunks == 0:
            raise ValueError(f"Data file '{path}' cannot produce any complete chunks of size {self._chunk_size}")
