...
from olmo.config import ModelConfig as OLMoModelConfig
from olmo.custom_hf_model_config import (
    CustomOlmo2Config,
    CustomOlmo2MoEConfig,
)
from olmo.model import BufferCache
from olmo.moe import DroplessMoE, MoE
...
class Olmo2DecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: CustomOlmo2Config, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
...
@auto_docstring
class CustomOlmo2Model(Olmo2PreTrainedModel):
    def __init__(self, config: CustomOlmo2Config):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        return BaseModelOutputWithPast(
            ...
            attentions=tuple(all_attn_weights),
        )


@auto_docstring
class CustomOlmo2ForCausalLM(Olmo2PreTrainedModel, GenerationMixin):
...
class Olmo2MoEDecoderLayer(GradientCheckpointingLayer):
    """MoE Decoder Layer combining attention and MoE MLP."""
    ...
    def __init__(self, config: CustomOlmo2Config, layer_idx: int):
        ...
        # Create OLMo model config for MoE
        olmo_config = self._create_olmo_config(config)
        moe_config = config.get_moe_config()

        # Create buffer cache for MoE
        self.cache = BufferCache()

        # Create MoE layer using the actual implementation
        if moe_config.moe_type.value == "dropless_moe":
            self.moe = DroplessMoE(
                model_config=olmo_config,
    ...
    def _create_olmo_config(
        ...
        olmo_config.d_model = config.hidden_size
        olmo_config.n_layers = config.num_hidden_layers
        olmo_config.init_device = "cpu"
        olmo_config.include_bias = config.attention_bias
...
@auto_docstring
class CustomOlmo2MoEModel(Olmo2PreTrainedModel):
    config: CustomOlmo2MoEConfig
    """Custom OLMo2 MoE Model for HuggingFace integration."""
...
@auto_docstring
class CustomOlmo2MoEForCausalLM(Olmo2PreTrainedModel, GenerationMixin):
    """Custom OLMo2 MoE Model for Causal Language Modeling."""
...
__all__ = [
    "CustomOlmo2ForCausalLM",
    "CustomOlmo2MoEForCausalLM",
    "CustomOlmo2MoEModel",
]

# Register the models with Transformers
# Regular OLMo2 models
AutoConfig.register("customolmo2", CustomOlmo2Config)
AutoModel.register(CustomOlmo2Config, CustomOlmo2Model)
AutoModelForCausalLM.register(CustomOlmo2Config, CustomOlmo2ForCausalLM)
