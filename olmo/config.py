...
class InitFnType(StrEnum):
    ...
    """
    All weights are initialized with the Xavier method from a uniform distribution.
    """

    xavier_normal = "xavier_normal"
    """
    All weights are initialized with the Xavier method from a normal distribution.
    """
...
class MoEConfig(BaseConfig):
    ...
    hidden_size: int = 384
    router_type: MoERouterType = MoERouterType.linear
    num_experts: int = 32
    top_k: int = 1
    ...
    gating_function: MoERouterGatingFunction = (
    ...
    moe_type: Optional[MoEType] = MoEType.dropless_moe


@dataclass
class SharedMLPConfig(BaseConfig):
...
class ModelConfig(BaseConfig):
    ...
    """
    whether to use peri_norm (https://arxiv.org/abs/2502.02732)
    """

    mix_norm_factor: Optional[float] = None
    """
    Factor to scale the layer norm by.
    """

    layer_norm_scaling: bool = False
    """
    Whether to scale the layer norm by the square root of the layer depth.
    """

    moe_config: Optional[MoEConfig] = None

    attn_minus: bool = False
    """
    Whether to subtract the mean of the attention weights.
    """

    mindgap: bool = False
    """
    Whether to subtract the mean of the value tensor.
    """
...
class OptimizerConfig(BaseConfig):
    ...
    """
    Whether to enable spike detection for tracking grad^2 / v statistics.
    """

    spike_threshold: float = 100.0
    """
    Threshold for spike detection. Parameters with grad^2 / v above this value are considered spikes.
    """

    spike_plot_interval: Optional[int] = None
    """
    How often (in steps) to generate spike plots. If None, defaults to save_interval.
    """

    track_step_history: bool = False
    """Whether to track the history of spike counts per step. Set to False to save memory if only current step analysis is needed."""
    ...
    def update_legacy_settings(cls, config: D) -> D:
        ...
        if om.is_dict(new_config):
            ...
            if (
                hasattr(new_config, "name")
                and new_config.name == "decoupled_lionw"
            ):
...
class SchedulerConfig(BaseConfig):
    ...
    """
    The starting LR during the decay period. If not set this defaults to 10% of
    the target LR.
    """

    decay_fraction: Optional[float] = None
    """
    The fraction of the total training steps for the decay period. If not set this defaults to 0.1.
    """
...
@dataclass
class LayerwiseMomentsConfig(BaseConfig):
    """
    Configuration for collecting higher order moments and detecting outlier features
    during training to understand variance explosion and Collapse of Depth (CoD).
    """

    collect_interval: int = 10
    """
    The interval at which to collect higher order moments.
    """

    collect_full_moments: bool = True
    """
    Whether to collect all higher order moments (skewness, kurtosis, etc.).
    If False, only basic statistics (mean, variance) will be collected.
    """

    outlier_detection_method: OutlierDetectionMethod = OutlierDetectionMethod.zscore
    """
    Method for detecting outlier features:
    - 'zscore': Features with |z-score| > threshold
    - 'quantile': Features in top/bottom percentile
    - 'topk': Top-K features by absolute magnitude
    """

    outlier_threshold: float = 3.0
    """
    Threshold for z-score based outlier detection.
    Features with |z-score| > threshold are considered outliers.
    """

    outlier_percentile: float = 0.01
    """
    Percentile for quantile-based outlier detection.
    For example, 0.01 means top 1% and bottom 1% are outliers.
    """

    top_k: int = 10
    """
    Number of top features to track for top-K outlier detection.
    """

    sample_features: bool = False
    """
    Whether to sample features for efficiency when computing expensive statistics.
    """

    sample_ratio: float = 0.1
    """
    Ratio of features to sample when sample_features is True.
    For example, 0.1 means sample 10% of features.
    """

    compute_per_feature_moments: bool = False
    """
    Whether to compute moments for each feature dimension separately.
    This provides d_model values for each moment type (e.g., 1536 skewness values).
    Useful for analyzing feature-level statistics but increases computation cost.
    """

    collect_distribution: bool = False
    """
    Whether to collect full feature distributions for visualization.
    This can be memory-intensive for large models.
    """

    distribution_sample_size: int = 10000
    """
    Maximum number of feature values to sample per layer per step when
    collect_distribution is True.
    """

    compute_channel_moments: bool = True
    """
    Whether to compute skewness and kurtosis in the channel dimension and average across tokens.
    This provides insights into how the distribution varies across the channel dimension
    for each token position, which is different from global flattened statistics.
    """
...
class DataConfig(BaseConfig):
    ...
    def effective_memmap_dtype(self):
        try:
            ...
            raise TypeError(
                f"Value {self.memmap_dtype} is not a valid numpy type"
            ) from e
        return dtype
...
class CustomDatasetCollatorConfig(BaseConfig):
    input_id_field: str = (
        "input_ids"  #: The field in the dataset items that contains the input token IDs.
    )
    attention_mask_field: Optional[str] = (
        None  #: The field in the dataset items that contains the attention mask.
    )
    attention_bias_field: Optional[str] = (
        None  #: The field in the dataset items that contains the attention bias.
    )
    label_mask_field: Optional[str] = (
        None  #: The field in the dataset items that contains the label mask.
    )
    index_field: Optional[str] = (
        None  #: The field in the dataset items that contains the index of the item.
    )
    instance_mask_field: Optional[str] = (
        None  #: The field in the dataset items that contains the instance mask.
    )
    doc_lens_field: Optional[str] = (
        None  #: The field in the dataset items that contains the document lengths.
    )
    metadata_field: Optional[str] = (
        None  #: The field in the dataset items that contains the metadata.
    )
...
class CustomDatasetConfig(BaseConfig):
    ...
    args: Optional[Dict[str, Any]] = (
        None  #: The arguments to pass to the custom dataset class or function
    )
    ...
    token_field: Optional[str] = (
        None  #: The field in the dataset items that contains the tokenized text.
    )
...
class TrainConfig(BaseConfig):
    ...
    save_ckpt_to_local: bool = True
    """
    If ``True``, save checkpoints to local.
    """

    save_ckpt_to_mos: bool = True
    """
    If ``True``, save checkpoints to MOS.
    """

    mos_version_uri: Optional[str] = None
    """
    The URI of the MOS version to save checkpoints to.
    """
    ...
    sharded_checkpointer: ShardedCheckpointerType = (
        ShardedCheckpointerType.torch_legacy
    )
    """
    ...
    device_train_batch_size: Optional[int] = (
    ...
    device_train_grad_accum: Optional[int] = (
        None  # calculated automatically
    )
    """
    ...
    """
    If set, gradient norms will be clipped using AdaCLIP.
    """
    ...
    swanlab: Optional[WandbConfig] = None
    """
    ...
    single: SingleGPUConfig = field(
        default_factory=lambda: SingleGPUConfig(device="auto")
    )
    """
    ...
    activation_checkpointing: Optional[ActivationCheckpointingStrategy] = (
        None
    )
    """
    ...
    layerwise_statis_collect_interval: Optional[int] = None
    """
    The interval at which to collect layerwise statistics.
    """

    layerwise_moments_config: Optional[LayerwiseMomentsConfig] = None
    """
    Configuration for collecting higher order moments and detecting outlier features.
    """
    ...
    def autocast_precision(self) -> torch.dtype:
        if self.precision == "amp_bf16":
            ...
            raise ValueError(
                f"Unexpected precision type '{self.precision}'"
            )
        ...
        if om.is_dict(new_config):
            ...
            if hasattr(new_config, "optimizer"):
                new_config.optimizer = (
                    OptimizerConfig.update_legacy_settings(
                        new_config.optimizer
                    )
                )
