...
.PHONY : lumi-image
lumi-image :
	docker build -f docker/Dockerfile.lumi -t anonymous/llm-lumi:$(shell git log -1 --pretty=format:%h) .
	docker push anonymous/llm-lumi:$(shell git log -1 --pretty=format:%h)

.PHONY : singularity-pull
singularity-pull :
	singularity pull $PROJECT_DIR/containers/llm-lumi_$(TAG).sif docker://anonymous/llm-lumi:$(TAG)
